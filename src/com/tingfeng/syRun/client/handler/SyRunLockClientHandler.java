 ...
 *
 */
public class SyRunLockClientHandler extends IoHandlerAdapter {
	
	private static final  SyRunLockClientHandler signleRunClientHandler = new SyRunLockClientHandler();	
	private SyRunLockClientHandler() {}
	
	public static SyRunLockClientHandler getSigleInstance(){
	...
	public  static String sendMessage(RequestBean<?> requestBean) throws InterruptedException, OutTimeException, IOException{
		IoSession session = SyRunLockTCPClient.syLockClient.get().getSession();
		return sendMessage(session,requestBean);
    }
	/**
	 ...
	 * @throws IOException 
	 */
	public static String sendMessage(IoSession session,RequestBean<?> requestBean) throws InterruptedException, OutTimeException, IOException{			
		String msg = JSONObject.toJSONString(requestBean);
		String returnMsg = "";
		WriteFuture future =null;
	    	future = session.write(msg);
	    	returnMsg = readReturnMsg(future,session);
	    	SyRunLockTCPClient.syLockClient.get().closeConnect();
		return returnMsg;
    }
